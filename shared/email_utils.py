...
class EmailClient:
    def __init__(
        ...
        self.__resend_from_email = resend_from_email
        self.__resend_api_key = resend_api_key
        self.__fallback_smtp_server = fallback_smtp_server
        self.__fallback_smtp_port = fallback_smtp_port
        self.__fallback_username = fallback_username
        self.__fallback_password = fallback_password
        self.__fallback_from_email = fallback_from_email
    ...
    def __send_email(self, recipient_email: str, subject: str, body: str):
        if can_send_with_resend():
            ...
            logger.info(f'Resend limit reached, falling back to {self.__fallback_smtp_server} for {recipient_email}')
    ...
    def __send_with_resend(self, recipient_email: str, subject: str, body: str):
        resend.api_key = self.__resend_api_key
        params: resend.Emails.SendParams = {
            'from': self.__resend_from_email,
            'to': [recipient_email],
            'subject': subject,
            'html': body,
        }
    ...
    def __send_with_fallback(self, recipient_email: str, subject: str, body: str):    
        ...
        msg['Subject'] = subject
        msg['From'] = self.__fallback_from_email
        msg['To'] = recipient_email
        msg.attach(MIMEText(body, 'html'))

        try:
            logger.debug(f'Connecting to SMTP server {self.__fallback_smtp_server}:{self.__fallback_smtp_port}')
            server = smtplib.SMTP(self.__fallback_smtp_server, self.__fallback_smtp_port)
            server.starttls()
            logger.info(f'Logging in as {self.__fallback_username} with passwrod [hidden]{self.__fallback_password[-3:]}')
            server.login(self.__fallback_username, self.__fallback_password)
            logger.info('Sending email to %s', recipient_email)
            server.sendmail(self.__fallback_from_email, recipient_email, msg.as_string())
            server.quit()
    ...
    def enqueue_send_activate_confirmation_email(self, recipient_email: str):
        ...
        subject, body = activation_email_content(self.__api_base_url, token)
    ...
    def enqueue_send_delete_confirmation_email(self, recipient_email: str):
        ...
        subject, body = deletion_email_content(self.__api_base_url, token)
        self.__enqueue_send(recipient_email, subject, body)
    ...
    def enqueue_send_pause_confirmation_email(self, recipient_email: str):
        ...
        subject, body = pause_email_content(self.__api_base_url, token)
        self.__enqueue_send(recipient_email, subject, body)
    ...
    def enqueue_send_resume_confirmation_email(self, recipient_email: str):
        ...
        subject, body = resume_email_content(self.__api_base_url, token)
        self.__enqueue_send(recipient_email, subject, body)
    ...
    def enqueue_send_notification_email(self, recipient_email: str, event_changes: list[EventChange]):
        ...
        token = create_token(recipient_email, 'pause')
        subject, body = notification_email_content(self.__api_base_url, event_changes, token)
        self.__enqueue_send(recipient_email, subject, body)
