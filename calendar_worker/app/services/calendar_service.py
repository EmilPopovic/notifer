...
class CalendarService:
    ...
    def __init__(self, s3_client: S3Client, email_client: EmailClient, base_calendar_url: str):
    ...
    def get_previous_calendar_content(self, subscription: UserCalendar) -> str | None:
        ...
        previous_content = self.s3_client.get_calendar(subscription.email)
    ...
    def save_calendar_to_s3(self, email: str, content: str) -> str | None:
        ...
        calendar_s3_url = self.s3_client.save_calendar(email, content)
    ...
    def build_calendar_url(self, email: str, calendar_auth: str) -> str:
        '''
        Get a calendar download link of the user with email and auth string.

        Args:
            email: email address of the user
            calendar_auth: the user's calendar auth string

        Returns:
            calendar download URL as string
        '''
        address = email.split('@')[0]
        return f'{self.base_calendar_url}?user={address}&auth={calendar_auth}'
    ...
    def process_subscription(self, subscription: UserCalendar) -> dict:
        ...
        try:
            ...
            # Build calendar URL
            calendar_url = self.build_calendar_url(subscription.email, subscription.calendar_auth)
